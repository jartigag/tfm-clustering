...
El objetivo de este trabajo se centra en la clasificación de los equipos de la red empresarial, así que se pondrá la atención sobre el segundo itinerario descrito, de color amarillo en la figura \ref{fig:diagramared}: el tráfico desde la red hacia Internet.
...
$ shuf -n 1000000 FORTINET_INTERNET_CORP_10.251.0.101.1 | \
    awk -F"···" '{print $1,$2,$5,$6,$7,$8,$9,$10,$11,$12,$13}' \
    > datasets_clustering/FORTINET_INTERNET_CORP_10.251.0.101.23may
\end{verbatim}

En las pruebas iniciales se descartaron las marcas de tiempo (campos \verb|$1,$2|) por simplicidad,
sabiendo que así se empezaría a trabajar con características más acotadas.
...
La manera en la que se expresarían como características requiere ser explicada con más profundidad en los párrafos sucesivos,
donde se detallará cómo se han calculado los valores, cómo se ha realizado la agregación y por qué se ha escogido usar estas métricas.
...
Hay ciertas variables que no tiene sentido resumir mediante un recuento, como son: los niveles de amenaza y anomalía, la prioridad máxima y las características temporales.
Esas variables [..]
Como resultado de esta transformación, se tiene una matriz con 11 columnas y \emph{n} filas, siendo \emph{n} el número de IPs de origen distintas presentes en la muestra de ese día.
...
\begin{table}[h]
    ...
    \begin{tabular}{|c|c|}
        \hline
        \verb|len(data[src_ip]['dst_ip'])| & Número de IPs destino únicas \\
        \hline
        \verb|len(data[src_ip]['proto'])| & Número de protocolos usados \\
        \hline
        \verb|len(data[src_ip]['src_port'])| & Número de puertos origen únicos \\
        \hline
        \verb|len(data[src_ip]['dst_port'])| & Número de puertos destino únicos \\
        \hline
        \verb|mean(data[src_ip]['anom_level'])| & Nivel de anomalía medio \\
        \hline
        \verb|mean(data[src_ip]['threat_level'])| & Nivel de amenaza medio \\
        \hline
        \verb|min(data[src_ip]['max_prio'])| & Prioridad máxima \\
        \hline
        \verb|sum(data[src_ip]['count_events']| & Número de eventos \\
        \hline
        \verb|avg_duration| & Duración de sesión media \\
    \hline
        \verb|end_tstamps| & Tiempos de finalización de sesiones \\
        \hline
    \end{tabular}
    \caption{Métricas con las que se resumen las sesiones en matrices diarias}
    \label{tab:metricas}
\end{table}

[Mejorar \ref{tab:metricas}. Explicar por qué se han elegido métricas como medias, stddevs?, etc.]
...
Las dos variables siguientes, que son los niveles de anomalía y amenaza, se hallan calculando la media de cada nivel en todo el periodo de tiempo bajo análisis (un día).
...
La variable \texttt{count\_events} es la suma de eventos que el firewall ha asociado a una misma IP origen.
Las últimas dos variables son las relativas al tiempo:
``duración de sesión media'' recoge cuántos segundos han durado de media las sesiones que ha mantenido una IP origen en el día en cuestión,
mientras que en ``end\_tstamps'' se almacena una lista con los \emph{timestamps} en los que ha acabado cada sesión de esa IP origen.
Por tanto, la longitud de esta lista es distinta para cada IP origen.

Procediendo análogamente sobre los datos de cada día, obtenidos entre el lunes 25 de mayo de 2020 y el domingo 31,
se tienen 7 matrices de 11 variables por unas 4800 filas (equivalentes al total de IPs origen distintas cada día),
...
\begin{figure}[h]
    ...
    %\subfigure[]{\includegraphics[width=0.3\textwidth]{contenido/fig/ecdf-count_dst_ip.pdf}}
    %\subfigure[]{\includegraphics[width=0.3\textwidth]{contenido/fig/ecdf-count_dst_port.pdf}}
    %\subfigure[]{\includegraphics[width=0.3\textwidth]{contenido/fig/ecdf-count_events.pdf}}
