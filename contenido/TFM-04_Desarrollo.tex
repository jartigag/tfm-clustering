...
La técnica de selección de $k$ por el método del codo consiste en representar en una gráfica lineal a la media respecto del número de \emph{clusters}.
...
Como se adelantaba en la sección \ref{sec:preprocesado}, en las primeras pruebas se incluyeron las características anotadas como
``dst\_ip'', ``proto'', ``src\_port'', ``dst\_port'', ``anom\_level'', ``max\_prio'', ``count\_events'' y ``avg\_duration''.
...
Se incluye también los modelos de cada \emph{cluster} que ofrece BigML, definidos mediante algunas reglas de clasificación.
Además, se listan las características que han contribuido a la formación de cada \emph{cluster} ordenadas por influencia.
