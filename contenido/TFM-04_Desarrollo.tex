...
\begin{itemize}
    \item Si se trata de tráfico que procede de los usuarios externos conectándose a servicios públicos del banco (expuestos a Internet), se cursa a través de una batería de equipos formada por: un IPS PaloAlto, un WAF\footnote{\emph{Web Application Firewall}} de marca Fortinet, balanceadores F5, otros firewalls Checkpoint y, finalmente, los servidores.
    \item En caso de ser tráfico desde la red corporativa hacia Internet, los equipos en este camino son el IPS PaloAlto mencionado (pero en esta ocasión está funcionando solo como IDS) y un firewall Fortinet, en este orden.
    \item Si son conexiones internas entre diferentes organizaciones, se procesa con otro firewall Fortinet independiente, etc.
\end{itemize}

\begin{figure}[h]
    \centering
    \captionsetup{width=12cm}
    \includegraphics[width=\textwidth]{contenido/fig/diagrama_red.pdf}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\textwidth]{contenido/fig/volumen_logs.pdf}
...
una serie de eventos asociados que se corresponden con la misma tupla de:
\begin{itemize}
    \item IP origen
    \item IP destino
    \item Protocolo
...
Hay ciertas variables que no tiene sentido resumir mediante un recuento, como son:
\begin{itemize}
    \item Protocolos usados
    \item Niveles de amenaza y anomalía
    \item Prioridad máxima
    \item Características temporales
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{contenido/fig/ecdf-count_dst_ip.pdf}
    \includegraphics[width=0.5\textwidth]{contenido/fig/ecdf-count_dst_port.pdf}
    \includegraphics[width=0.5\textwidth]{contenido/fig/ecdf-count_events.pdf}
...
\begin{figure}[h]
    ...
    \includegraphics[width=\linewidth]{contenido/fig/dispersion-k3-dst_ip-vs-avg_duration.pdf}
...
\begin{figure}[h]
    \centering
    \captionsetup{width=0.75\textwidth}
    \includegraphics[width=\linewidth]{contenido/fig/dispersion-k5-dst_ip-vs-count_events.pdf}
...
se ha repetido el último ensayo (indicado en la subsección \ref{subsec:ensayoD}) manteniendo solo las 6 características más influyentes:
\begin{itemize}
    \item ``dst\_ip''
    \item ``proto''
    \item ``src\_port''
...
\begin{figure}[h!]
    \centering
    \captionsetup{width=0.75\textwidth}
    \includegraphics[width=\textwidth]{contenido/fig/codo.pdf}
...
Se usaron las características anotadas como:
\begin{itemize}
    \item ``dst\_ip''
    \item ``proto''
    \item ``src\_port''
...
Sirva como ejemplo la segunda regla que se ha extraído en el \emph{cluster} de ``Comportamiento normal, pocas conexiones''.
